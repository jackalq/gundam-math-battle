Object.assign(WarScene.prototype,{
  ...
  attackPlan(attacker,target,min,max,enemyShot){
    return {attacker,target,min,max,enemyShot,weapon:this.weaponFor(attacker,target)};
  },
  async animateAttackPlan(plan){
    const {attacker,target,enemyShot,weapon}=plan,a=this.elFor(attacker),t=this.elFor(target);
    if(!a||!t)return;
    a.classList.add(weapon.type==='melee'?'using-melee':'using-ranged');
    if(weapon.type==='melee')await WeaponFX.melee(this.field,a,t,enemyShot);
    else{a.classList.add('firing');await WeaponFX.ranged(this.field,a,t,enemyShot,{burst:enemyShot?3:1})}
  },
  async performVolley(plans,label='全機攻擊'){
    const active=plans.filter(p=>p&&WarRoles.Target.alive(p.attacker)&&WarRoles.Target.alive(p.target)).map(p=>this.attackPlan(p.attacker,p.target,p.min,p.max,p.enemyShot));
    ...
    // Lock every attacker/target before animation so the whole formation attacks at once.
    ...
    await Promise.all(impacted.map(t=>{const e=this.elFor(t);return e?this.explode(e,t.hp<=0):Promise.resolve()}));
    await delay(170);
    ...
    const down=impacted.filter(t=>!t.alive),total=[...targetSummary.values()].reduce((n,x)=>n+x.damage,0);
